...
SELECT `film`.`film_id`, `film`.`title`, COUNT(`film_actor`.`actor_id`) AS `Number of Actors in Film`
FROM `film`
JOIN `film_actor`
...
SELECT COUNT(`inventory`.`inventory_id`) `Copies of Hunchback Impossible`
FROM `inventory`
JOIN `film`
...
SELECT `store`.`store_id`, `address`.`address`, SUM(`payment`.`amount`) AS `Total Sales`
FROM `store`
JOIN `address`
...
AS SELECT `category`.`name`, SUM(`payment`.`amount`) AS `Gross Revenue`
FROM `category`
JOIN `film_category`
  ON `film_category`.`category_id` = `category`.`category_id`
JOIN `inventory`
